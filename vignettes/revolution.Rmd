...
```{r}
method <- readline(prompt = "If you use Windows or MacOS, please type 'auto'. For linux users: type 'wget'")
update_rki_data(method=method)
```
## Loading and updating the underlying data

The "revolution" package offers the possibility to manually update the underlying data, so that the plots of time series like the 7-day-incidence or the vaccination data are always up to date. To update official data provided by the Robert Koch Institut, the function `update_rki_data()` can be called.
