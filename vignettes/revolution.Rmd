...
The function `calc_distribution_report_diff()` calculates the temporal difference between infection and reporting date for each traced case. This is nicely visualized by `plot_distribution_report_diff()`:

```{r, fig.width=7}
plot_distribution_report_diff()
```

As it can be seen, most of the cases were reported just a few days after the assumed infection date, but a not neglectable share of cases shows a big temporal difference between infection and reporting. In approximately more than 30 000 cases, the reporting date is even before the assumed infection date. That doesn't make sense at all. 

As a conclusion, one can say that the data needs to be interpreted and analyized carefully.
