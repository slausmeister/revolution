...
office_case_distribution <- function(bundesland_1 = "Baden-Württemberg", bundesland_2 = "Schleswig-Holstein"){
  ...
  rki_data %>%
    ...
    dplyr::summarize(count_weekday = n()) -> weekday_distribution
  total_rows <- dim(rki_data)[1]
  weekday_distribution %>%
    dplyr::mutate(across(count_weekday, ~ ./total_rows)) -> mean_distribution
  ...
  rki_data %>%
    ...
    dplyr::summarize(count_weekday = n()) -> bundesland_distribution

  bundesland_distribution %>% dplyr::filter(Bundesland == bundesland_1) %>%
    dplyr::mutate(across(count_weekday, ~ ./sum(count_weekday))) -> bundesland_1
  bundesland_1

  bundesland_distribution %>% dplyr::filter(Bundesland == bundesland_2) %>%
    dplyr::mutate(across(count_weekday, ~ ./sum(count_weekday))) -> bundesland_2
  bundesland_2

  office_case_distribution_plot <- ggplot()+
    geom_line(data=mean_distribution, aes(x = weekday, y = count_weekday, group = 1, color= "Durchschnitt"))+
    geom_line(data=bundesland_1, aes(x = weekday, y = count_weekday, group = 1, color=Bundesland))+
    geom_line(data=bundesland_2, aes(x = weekday, y = count_weekday, group = 1, color=Bundesland))
